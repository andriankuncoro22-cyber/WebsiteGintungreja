...
const potensiSubLinks = [
  ...
  { href: '/potensi-desa?tab=bumdes', label: 'BUMDes Gintungreja' },
  ...
  return (
    <header
      className={cn(
        ...
        isSolid
          ? 'border-amber-900/10 bg-[#FAF6EF]/90 backdrop-blur-xl shadow-[0_8px_30px_rgba(40,30,20,0.06)]'
          : 'border-transparent bg-transparent backdrop-blur-none'
      )}
    >
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        <Link href="/" className={cn('transition-colors', isSolid ? 'text-stone-900' : 'text-white')} aria-label="Beranda Portal Portal Desa Gintungreja">
          <Logo />
        </Link>
        ...
        <nav className="hidden flex-1 items-center justify-center gap-3 xl:gap-4 md:flex">
          <Link
            ...
            className={cn(
              ...
              isSolid ? 'text-stone-800 hover:text-amber-800' : 'text-white/90 hover:text-white'
            )}
          >
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="center" className="mt-3 w-56 rounded-xl border border-amber-900/10 bg-[#FFFDF9]/95 backdrop-blur-xl p-2 shadow-xl shadow-stone-200/50">
              {statistikSubLinks.map((link) => (
                <DropdownMenuItem key={link.href} asChild className="rounded-lg px-4 py-2 text-[11px] font-semibold uppercase tracking-[0.12em] text-stone-600 transition-colors hover:text-amber-800 hover:bg-amber-50/80 cursor-pointer">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="center" className="mt-3 w-60 rounded-xl border border-amber-900/10 bg-[#FFFDF9]/95 backdrop-blur-xl p-2 shadow-xl shadow-stone-200/50">
              {potensiSubLinks.map((link) => (
                <DropdownMenuItem key={link.href} asChild className="rounded-lg px-4 py-2.5 text-[11px] font-semibold uppercase tracking-[0.12em] text-stone-600 transition-colors hover:text-amber-800 hover:bg-amber-50/80 cursor-pointer">
                  <Link href={link.href}>{link.label}</Link>
                </DropdownMenuItem>
              ))}
            </DropdownMenuContent>
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="center" className="mt-3 w-52 rounded-xl border border-amber-900/10 bg-[#FFFDF9]/95 backdrop-blur-xl p-2 shadow-xl shadow-stone-200/50">
        ...
        <div className="hidden items-center gap-2 lg:flex">
          <Link href="/login/" aria-label="Masuk area admin">
            <Button variant="outline" size="sm" className={cn(
              ...
              isSolid
                ? 'border-stone-300 bg-white/90 text-stone-700 hover:bg-stone-100'
          ...
          <Link href="/layanan-surat/" aria-label="Ajukan layanan desa">
            <Button className="h-7 rounded-full bg-gradient-to-r from-amber-700 via-orange-700 to-amber-800 px-3 text-[9px] font-semibold text-white shadow-[0_4px_16px_rgba(194,94,52,0.25)] transition-all duration-300 hover:-translate-y-0.5 hover:from-amber-800 hover:to-orange-800">
              Ajukan Layanan
            </Button>
          </Link>
        </div>
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 lg:hidden">
          <Link href="/login/" aria-label="Masuk area admin">
            <Button
              ...
              className={cn(
                ...
                isSolid
                  ? 'border-stone-300/80 bg-white/90 text-stone-700 hover:bg-stone-100 hover:text-amber-700 shadow-xs'
          ...
          <Sheet open={mobileSheetOpen} onOpenChange={setMobileSheetOpen}>
            <SheetTrigger asChild>
              <Button variant="ghost" size="icon" className={cn('rounded-full h-8 w-8 sm:h-9 sm:w-9 flex items-center justify-center', isSolid ? 'text-stone-700 hover:bg-stone-100' : 'text-white hover:bg-white/10')} aria-label="Buka menu navigasi">
                <Menu className="h-5 w-5" />
              </Button>
            </SheetTrigger>
            <SheetContent side="right" className="w-[300px] sm:w-[340px] max-w-[85vw] border-l border-amber-900/20 bg-[#1C1613] text-[#FAF6EF] p-4 sm:p-6">
              ...
              <div className="mt-4 flex flex-col gap-4 h-[calc(100vh-64px)] overflow-y-auto pb-12 pr-1 no-scrollbar">
                ...
                <div className="space-y-2 mt-2">
                  <Link href="/layanan-surat/" onClick={() => setMobileSheetOpen(false)} className="block">
                    <Button className="h-10 sm:h-12 w-full rounded-xl sm:rounded-full bg-gradient-to-r from-amber-700 to-orange-700 hover:from-amber-800 hover:to-orange-800 text-white font-bold text-xs shadow-md">
                      Ajukan Layanan
