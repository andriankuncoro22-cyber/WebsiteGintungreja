...
const serviceItemsRow1: ServiceCardItem[] = [
  {
    ...
    href: '/layanan-surat/',
    icon: <FileCheck2 className="h-6 w-6 text-amber-100" />,
    vector: (
  ...
  },
  {
    ...
    href: '/pelayanan-desa/',
    icon: <FileText className="h-6 w-6 text-amber-100" />,
    vector: (
      <svg viewBox="0 0 100 100" fill="none" stroke="currentColor" className="w-full h-full">
        {/* Papan Klip SOP */}
  ...
  },
  {
    ...
    href: '/#pbb-section',
    icon: <Receipt className="h-6 w-6 text-amber-100" />,
    vector: (
      <svg viewBox="0 0 100 100" fill="none" stroke="currentColor" className="w-full h-full">
  ...
  },
  {
    ...
    href: '/pengaduan/',
    icon: <MessageSquareQuote className="h-6 w-6 text-amber-100" />,
    vector: (
      <svg viewBox="0 0 100 100" fill="none" stroke="currentColor" className="w-full h-full">
  ...
  },
  {
    ...
    href: '/statistik/',
    icon: <BarChart3 className="h-6 w-6 text-amber-100" />,
    vector: (
      <svg viewBox="0 0 100 100" fill="none" stroke="currentColor" className="w-full h-full">
        {/* Sumbu Grafik */}
...
const serviceItemsRow2: ServiceCardItem[] = [
  {
    ...
    icon: <Store className="h-6 w-6 text-amber-100" />,
    vector: (
      <svg viewBox="0 0 100 100" fill="none" stroke="currentColor" className="w-full h-full">
        {/* Kanopi Toko UMKM */}
  ...
  },
  {
    ...
    icon: <ShieldCheck className="h-6 w-6 text-amber-100" />,
    vector: (
      <svg viewBox="0 0 100 100" fill="none" stroke="currentColor" className="w-full h-full">
  ...
  },
  {
    ...
    icon: <Landmark className="h-6 w-6 text-amber-100" />,
    vector: (
      <svg viewBox="0 0 100 100" fill="none" stroke="currentColor" className="w-full h-full">
  ...
  },
  {
    ...
    href: '/potensi-desa/',
    icon: <Compass className="h-6 w-6 text-amber-100" />,
    vector: (
      <svg viewBox="0 0 100 100" fill="none" stroke="currentColor" className="w-full h-full">
  ...
  },
  {
    ...
    href: '/nomor-penting/',
    icon: <PhoneCall className="h-6 w-6 text-amber-100" />,
    vector: (
      <svg viewBox="0 0 100 100" fill="none" stroke="currentColor" className="w-full h-full">
...
export function ServicesSection() {
  ...
  return (
    <section className="mx-auto max-w-7xl px-3 sm:px-6 sm:py-16 lg:px-8 py-8">
      {/* KONTINER UTAMA (Warm Charred Teakwood & Bronze Foundry) */}
      <div className="relative overflow-hidden rounded-2xl sm:rounded-3xl bg-gradient-to-br from-[#1C1613] via-[#2A1E17] to-[#150F0C] p-2.5 min-[380px]:p-3.5 sm:p-10 lg:p-12 shadow-2xl border border-amber-900/30">
        {/* Background Network Mesh / Glow Lines Warm Gold */}
        <div className="absolute inset-0 pointer-events-none overflow-hidden select-none">
          <svg className="absolute -right-20 -top-20 w-[600px] h-[600px] opacity-15" viewBox="0 0 600 600" fill="none">
            <circle cx="300" cy="300" r="250" stroke="#D8902A" strokeWidth="1" strokeDasharray="4 4" />
            <circle cx="300" cy="300" r="180" stroke="#E5A125" strokeWidth="1.5" />
            <circle cx="300" cy="300" r="100" stroke="#D8902A" strokeWidth="1" strokeDasharray="3 3" />
            <line x1="100" y1="100" x2="500" y2="500" stroke="#D8902A" strokeWidth="0.75" />
            <line x1="500" y1="100" x2="100" y2="500" stroke="#D8902A" strokeWidth="0.75" />
          </svg>
          <div className="absolute -top-32 -left-32 w-96 h-96 rounded-full bg-amber-500/10 blur-3xl" />
          <div className="absolute -bottom-32 -right-32 w-96 h-96 rounded-full bg-orange-600/15 blur-3xl" />
        </div>

        {/* HEADER LAYANAN */}
        <div className="relative z-10 mb-4 sm:mb-10">
          <div className="inline-flex items-center gap-1.5 sm:gap-2 px-2.5 py-0.5 sm:px-3 sm:py-1 rounded-full bg-amber-500/20 border border-amber-400/30 text-amber-200 text-[10px] sm:text-[11px] font-bold uppercase tracking-wider mb-1.5 sm:mb-3 backdrop-blur-sm">
          ...
          <p className="mt-1 sm:mt-2.5 text-[11px] sm:text-base text-amber-100/80 max-w-2xl font-medium leading-relaxed">
        ...
        {/* 10 KARTU BENTO ARSITEKTURAL (Mobile: 4 per baris compact, Desktop: 5 per baris) */}
        <div className="relative z-10 grid grid-cols-4 sm:grid-cols-3 lg:grid-cols-5 gap-2 min-[380px]:gap-2.5 sm:gap-4 lg:gap-5">
          {allServiceItems.map((item, idx) => (
            ...
            >
              <Link
                href={item.href}
                className="group relative flex flex-col justify-between rounded-tl-xl rounded-br-xl rounded-tr-sm rounded-bl-sm sm:rounded-tl-[2rem] sm:rounded-br-[2rem] sm:rounded-tr-lg sm:rounded-bl-lg bg-gradient-to-b from-[#8F3714] via-[#A8451D] to-[#6E280C] p-1.5 min-[380px]:p-2 sm:p-5 text-white shadow-md sm:shadow-xl border-2 border-amber-400/30 ring-1 ring-amber-500/20 transition-all duration-300 hover:-translate-y-1.5 hover:border-amber-300 hover:shadow-2xl hover:shadow-amber-950/80 overflow-hidden aspect-square"
              >
                {/* Elemen Vektor Tematik Latar Belakang (Mencerminkan Judul Kartu) */}
                <div className="absolute right-0 top-0.5 sm:right-0 sm:top-9 w-8 h-8 min-[380px]:w-10 min-[380px]:h-10 sm:w-28 sm:h-28 pointer-events-none select-none text-amber-100 opacity-15 sm:opacity-25 group-hover:opacity-45 group-hover:scale-110 group-hover:-rotate-3 transition-all duration-500 ease-out">
                  {item.vector}
                </div>
                ...
                <div className="relative z-10 flex items-start justify-between">
                  <div className="flex h-6 w-6 min-[380px]:h-7 min-[380px]:w-7 sm:h-12 sm:w-12 items-center justify-center rounded-lg sm:rounded-2xl bg-white/15 border border-white/25 backdrop-blur-md shadow-inner transition-transform duration-300 group-hover:scale-110 group-hover:bg-white/25">
                  ...
                  <div className="hidden sm:flex h-6 w-6 sm:h-8 sm:w-8 items-center justify-center rounded-full border border-white/50 text-white transition-all duration-300 group-hover:border-white group-hover:bg-white group-hover:text-[#A8451D] group-hover:scale-110 shadow-sm">
                ...
                <div className="relative z-10 mt-auto">
                  <h3 className="text-[7.5px] min-[380px]:text-[8.5px] sm:text-[13px] font-bold sm:font-black uppercase tracking-tight text-white leading-[1.1] sm:leading-snug line-clamp-2 font-display">
                    {item.title}
                  </h3>
                  <p className="hidden sm:block mt-0.5 sm:mt-1 text-[9.5px] sm:text-xs leading-tight sm:leading-snug text-amber-100/90 font-normal line-clamp-2">
                    {item.subtitle}
                  </p>
                </div>
              </Link>
