...
const DEMO_PBB_RECORDS: Record<string, PbbRecord> = {
  '330107000100100010': {
    ...
    alamatPemilikTanah: 'RT 02 / RW 01 DUSUN GINTUNGREJA',
    nop: '33.01.070.001.001-0001.0',
    tahun: 2026,
    namaWp: 'SUWARNO HADI',
    alamatObjek: 'DUSUN GINTUNGREJA RT 02 RW 01',
    alamatWp: 'DUSUN GINTUNGREJA RT 02 RW 01',
    luasBumi: 420,
    luasBangunan: 96,
    njopBumi: 84000000,
...
export function PbbSearchSection() {
  ...
  const handleSearch = async (e?: React.FormEvent, customNop?: string) => {
    ...
    try {
      ...
      if (json.status === 'success' && json.data) {
        ...
        setErrorMessage(json.message || 'Nomor Objek Pajak (NOP) tidak ditemukan dalam data PBB Desa Gintungreja.');
      }
    } catch (err: any) {
  ...
  const handleWhatsApp = (record: PbbRecord) => {
    ...
    const statusText = isPaid
      ...
      : 'BELUM LUNAS';
    const message = `Halo Petugas PBB Desa Gintungreja,%0A%0ASaya ingin konfirmasi data SPPT PBB-P2:%0A- NOP: ${record.nop}%0A- Nama WP: ${record.namaWp}%0A- Alamat Objek: ${record.alamatObjek}%0A- PBB Dibayar: ${formatRupiah(record.pbbYangDibayar)}%0A- Status Terdata: ${statusText}%0A%0AMohon informasi lebih lanjut. Terima kasih.`;
  ...
  return (
    <section id="cek-pbb" className="relative mx-auto max-w-7xl px-3.5 py-8 sm:px-6 sm:py-12 lg:px-8 scroll-mt-24">
      <div className="relative rounded-2xl sm:rounded-[3.5rem] bg-gradient-to-b from-[#16110E] via-[#241A14] to-[#16110E] text-white p-4 sm:p-12 lg:p-16 shadow-2xl border-2 border-amber-900/40 ring-1 ring-amber-500/20 overflow-hidden">
        {/* Decorative Traditional Corner Vector Brackets */}
        <div className="absolute top-4 left-4 w-8 h-8 border-t-2 border-l-2 border-amber-500/40 rounded-tl-lg pointer-events-none" />
        <div className="absolute top-4 right-4 w-8 h-8 border-t-2 border-r-2 border-amber-500/40 rounded-tr-lg pointer-events-none" />
        <div className="absolute bottom-4 left-4 w-8 h-8 border-b-2 border-l-2 border-amber-500/40 rounded-bl-lg pointer-events-none" />
        <div className="absolute bottom-4 right-4 w-8 h-8 border-b-2 border-r-2 border-amber-500/40 rounded-br-lg pointer-events-none" />

        {/* Background Glow */}
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-amber-900/20 via-[#1F1713] to-[#16110E] pointer-events-none" />
        ...
        <div className="relative mx-auto max-w-5xl">
          ...
          <div className="text-center space-y-2 sm:space-y-4 max-w-3xl mx-auto mb-6 sm:mb-10">
            <div className="inline-flex items-center gap-1.5 sm:gap-2 rounded-full border border-amber-500/30 bg-amber-500/15 px-3 py-1 sm:px-4 sm:py-1.5 text-[10px] sm:text-xs font-bold uppercase tracking-widest text-amber-300">
              <Receipt className="h-3.5 w-3.5 sm:h-4 sm:w-4" />
              Layanan Perpajakan Desa
            </div>
            <h2 className="text-2xl sm:text-4xl md:text-5xl font-black tracking-tight font-display uppercase leading-tight text-amber-50">
              Cek Pajak <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-400 via-orange-300 to-amber-200">PBB-P2</span> Online
            </h2>
            <p className="text-stone-300 text-xs sm:text-base leading-relaxed">
              Pengecekan tagihan, rincian objek pajak, dan status pelunasan Pajak Bumi & Bangunan (PBB-P2) warga Desa Gintungreja secara transparan dan real-time langsung dari data SPPT desa.
            </p>
          </div>

          {/* Search Box Card — Vault Card with Double Border */}
          <div className="bg-[#1F1713]/80 backdrop-blur-xl border-2 border-amber-500/30 ring-1 ring-amber-500/15 p-3.5 sm:p-8 rounded-2xl sm:rounded-[2.5rem] shadow-2xl mb-8 sm:mb-12">
            <form onSubmit={(e) => handleSearch(e)} className="space-y-3 sm:space-y-4">
              <div className="relative flex flex-col sm:flex-row gap-2.5 sm:gap-3">
                <div className="relative flex-1">
                  <Search className="absolute left-3.5 sm:left-4 top-1/2 -translate-y-1/2 h-4 w-4 sm:h-5 sm:w-5 text-stone-400" />
                  <Input
                    type="text"
                    value={nopInput}
                    onChange={(e) => setNopInput(e.target.value)}
                    placeholder="Masukkan NOP Anda (misal: 33.01.070...)"
                    className="w-full h-12 sm:h-14 pl-10 pr-9 sm:pl-12 sm:pr-10 rounded-xl sm:rounded-2xl bg-white text-stone-900 placeholder:text-stone-400 font-semibold border-none focus-visible:ring-2 focus-visible:ring-amber-500 text-sm sm:text-base shadow-inner"
                  />
                  {nopInput && (
                    <button
                      type="button"
                ...
                <Button
                  type="submit"
                  disabled={isLoading}
                  className="h-12 sm:h-14 px-6 sm:px-8 rounded-xl sm:rounded-2xl bg-gradient-to-r from-[#C45528] via-[#D8902A] to-[#A8431B] hover:from-[#A8431B] hover:to-[#8F3512] text-white font-black text-xs sm:text-sm uppercase tracking-wider shadow-lg shadow-amber-900/40 transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] shrink-0"
                >
                  {isLoading ? (
                    <>
              ...
              <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 text-[11px] sm:text-xs text-stone-300 pt-1 sm:pt-2 px-1">
                <HelpCircle className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-amber-400 shrink-0" />
                <span>
                  <strong>Contoh format NOP:</strong>{' '}
                  <code className="bg-white/10 px-1.5 py-0.5 rounded text-amber-300 font-mono text-[10px] sm:text-xs">33.01.070.008.001.0016.0</code>{' '}
                  <span className="text-stone-400 hidden sm:inline">(dapat diketik angka atau dengan tanda titik)</span>
                </span>
              </div>
            </form>
          </div>
          ...
          <AnimatePresence mode="wait">
            {isLoading && (
              ...
              >
                <div className="relative mx-auto w-16 h-16">
                  ...
                  <Receipt className="absolute inset-0 m-auto h-6 w-6 text-amber-400 animate-pulse" />
                </div>
                <div className="space-y-1">
                  <h4 className="text-base font-bold text-white uppercase tracking-wider">Menghubungkan ke Data PBB Desa...</h4>
                  <p className="text-xs text-stone-400">Mencari baris NOP pada basis data Desa Gintungreja</p>
                </div>
              </motion.div>
            )}
            ...
            {!isLoading && hasSearched && errorMessage && (
              ...
              >
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-sm text-stone-300">{errorMessage}</p>
                </div>
                <div className="pt-2">
                  <Button
                    variant="outline"
            ...
            {!isLoading && searchResult && (
              ...
              >
                ...
                <Card className="rounded-[2.5rem] bg-[#FFFDF9] text-stone-900 border-none shadow-2xl overflow-hidden print:shadow-none print:m-0">
                  {/* Header Kartu SPPT */}
                  <div className="p-6 md:p-8 bg-gradient-to-r from-[#16110E] via-[#231A15] to-[#3B2519] text-white border-b border-white/10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                    ...
                    <div className="flex flex-col items-start md:items-end gap-2 shrink-0">
                      <span className="text-[10px] font-black uppercase tracking-widest text-stone-400">Status Pembayaran</span>
                      {isLunas ? (
                        <div className="inline-flex items-center gap-2 px-5 py-2.5 rounded-2xl bg-gradient-to-r from-amber-600 to-amber-700 text-white font-black text-sm uppercase tracking-wider shadow-lg shadow-amber-600/30">
                  ...
                  <CardContent className="p-6 md:p-10 space-y-8 font-sans">
                    ...
                    <div className="grid md:grid-cols-2 gap-8">
                      ...
                      <div className="space-y-6">
                        ...
                        <div className="space-y-4 text-sm">
                          <div className="bg-amber-50/50 p-4 rounded-2xl border border-amber-900/10 space-y-1">
                            <p className="text-[10px] font-bold uppercase tracking-wider text-stone-500">Nama Wajib Pajak (SPPT)</p>
                            <p className="text-lg font-black text-stone-900 uppercase font-display">{searchResult.namaWp || '-'}</p>
                            {searchResult.namaPemilikTanah && searchResult.namaPemilikTanah !== searchResult.namaWp && (
                              <p className="text-xs text-stone-500 font-medium">
                                Pemilik Tanah: <span className="font-bold text-stone-700">{searchResult.namaPemilikTanah}</span>
                              </p>
                            )}
                          </div>
                          ...
                          <div className="space-y-3">
                            <div className="flex flex-col gap-0.5">
                              <span className="text-[10px] font-bold uppercase tracking-wider text-stone-500">Nomor Objek Pajak (NOP)</span>
                              <span className="font-mono text-base font-bold text-stone-900 bg-stone-100 px-3 py-1.5 rounded-xl self-start border border-stone-200/80">
                                {searchResult.nop}
                              </span>
                            </div>
                            ...
                            <div className="flex flex-col gap-0.5">
                              <span className="text-[10px] font-bold uppercase tracking-wider text-stone-500 flex items-center gap-1">
                                <MapPin className="h-3 w-3 text-stone-400" /> Letak Objek Pajak
                              </span>
                              <span className="font-semibold text-stone-800 leading-snug">{searchResult.alamatObjek || '-'}</span>
                            </div>

                            {searchResult.alamatWp && (
                              <div className="flex flex-col gap-0.5">
                                <span className="text-[10px] font-bold uppercase tracking-wider text-stone-500">Alamat Wajib Pajak</span>
                                <span className="text-stone-600">{searchResult.alamatWp}</span>
                              </div>
                            )}

                            <div className="grid grid-cols-2 gap-3 pt-2">
                              <div className="bg-amber-50/80 p-3 rounded-xl border border-amber-200/80">
                                <p className="text-[10px] font-bold uppercase tracking-wider text-amber-900">Luas Bumi (Tanah)</p>
                                <p className="text-base font-black text-amber-950">{searchResult.luasBumi || 0} m²</p>
                              </div>
                              <div className="bg-orange-50/80 p-3 rounded-xl border border-orange-200/80">
                                <p className="text-[10px] font-bold uppercase tracking-wider text-orange-900">Luas Bangunan</p>
                                <p className="text-base font-black text-orange-950">{searchResult.luasBangunan || 0} m²</p>
                              </div>
                            </div>
                          </div>
                        </div>
                      ...
                      <div className="space-y-6">
                        ...
                        <div className="space-y-3 text-sm">
                          <div className="flex justify-between items-center py-2 border-b border-stone-100">
                            <span className="text-stone-500">NJOP Bumi:</span>
                            <span className="font-semibold text-stone-800">{formatRupiah(searchResult.njopBumi)}</span>
                          </div>

                          <div className="flex justify-between items-center py-2 border-b border-stone-100">
                            <span className="text-stone-500">NJOP Bangunan:</span>
                            <span className="font-semibold text-stone-800">{formatRupiah(searchResult.njopBangunan)}</span>
                          </div>

                          <div className="flex justify-between items-center py-2 border-b border-stone-100">
                            <span className="text-stone-500">NJOP Total SPPT:</span>
                            <span className="font-bold text-stone-900">{formatRupiah(searchResult.njopSppt)}</span>
                          </div>

                          <div className="flex justify-between items-center py-2 border-b border-stone-100">
                            <span className="text-stone-500">Pokok PBB Yang Dibayar:</span>
                            <span className="font-bold text-stone-900">{formatRupiah(searchResult.pbbYangDibayar)}</span>
                          </div>

                          {Boolean(Number(searchResult.denda)) && (
                            <div className="flex justify-between items-center py-2 border-b border-stone-100 text-rose-600">
                          ...
                          {Boolean(Number(searchResult.tunggakanTahunLalu)) && (
                            <div className="flex justify-between items-center py-2 border-b border-stone-100 text-amber-600">
                          ...
                          <div className="bg-amber-50/50 p-4 rounded-2xl border border-amber-900/10 space-y-2 text-xs">
                            {searchResult.petugasPemungut && (
                              <div className="flex justify-between">
                                <span className="text-stone-500">Petugas Pemungut (Kadus):</span>
                                <span className="font-bold text-stone-700">{searchResult.petugasPemungut}</span>
                              </div>
                            )}
                            {searchResult.tanggalBayar && (
                              <div className="flex justify-between">
                                <span className="text-stone-500">Tanggal Pelunasan:</span>
                                <span className="font-bold text-amber-800">{searchResult.tanggalBayar}</span>
                              </div>
                            )}
                          </div>
                        </div>
                    ...
                    {/* Action Buttons Bar */}
                    <div className="pt-6 border-t border-stone-100 flex flex-wrap items-center justify-between gap-4 print:hidden">
                      <div className="flex items-center gap-2 text-xs text-stone-400">
                        <Calendar className="h-4 w-4 text-amber-700" />
                        <span>Data diperbarui secara berkala oleh Tim Pemungut PBB Desa Gintungreja</span>
                      </div>

                      <div className="flex flex-wrap items-center gap-3">
                        <Button
                          onClick={() => handleWhatsApp(searchResult)}
                          className="rounded-xl gap-2 font-bold text-xs bg-gradient-to-r from-amber-700 to-orange-700 hover:from-amber-800 hover:to-orange-800 text-white shadow-md shadow-amber-900/20"
                        >
