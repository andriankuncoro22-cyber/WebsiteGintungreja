...
export function VisitorCounter() {
  ...
  useEffect(() => {
    ...
    const recordVisit = async () => {
      try {
        ...
        const sessionKey = `Gintungreja_visit_${todayStr}`;
  ...
  return (
    <div className="inline-flex flex-wrap items-center gap-2 sm:gap-3 rounded-xl sm:rounded-full bg-[#1C1613]/90 border border-amber-500/25 px-3.5 py-1.5 sm:px-4 sm:py-2 backdrop-blur-md shadow-lg text-xs">
      ...
      <div className="flex items-center gap-2 pr-1">
        ...
        <span className="text-[10px] sm:text-[11px] font-bold uppercase tracking-wider text-stone-300">
          Pengunjung
        </span>
        <span className="text-[8px] sm:text-[9px] font-semibold uppercase tracking-wider text-amber-400 bg-amber-500/10 border border-amber-500/20 px-1.5 py-0.5 rounded-full flex items-center gap-1">
          <Radio className="h-2 w-2 animate-pulse text-amber-400" />
          Live
        </span>
      </div>
      ...
      {/* Total */}
      <div className="flex items-center gap-1.5 pl-1 text-stone-400">
        <Globe className="h-3 w-3 text-[#C45528] shrink-0" />
        <span className="text-[10px] sm:text-[11px]">Total:</span>
        <span className="font-mono font-black text-amber-400 text-[11px] sm:text-xs">
