  ...
  const handleFilesChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      for (let i = 0; i < files.length; i++) {
        ...
        const secureUrl = await uploadToCloudinary(processingFile, 'umkm-gintungreja');
        urls.push(secureUrl);
      }
      ...
      <DialogContent className="w-[95vw] sm:max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl sm:rounded-3xl border border-slate-100 shadow-2xl p-4 sm:p-6 md:p-8">
        <DialogHeader>
          ...
          <DialogDescription className="text-xs sm:text-sm text-slate-500 font-medium">
            Formulir untuk memuat ulasan, potensi, pelatihan, atau profil sentra UMKM di Desa Gintungreja.
          </DialogDescription>
        </DialogHeader>
