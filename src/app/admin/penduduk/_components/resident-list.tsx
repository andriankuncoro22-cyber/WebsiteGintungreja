...
export function ResidentList() {
  ...
  const handleDownloadExcel = async () => {
    ...
    try {
      ...
      const excelData = dataToExport.map((r, index) => ({
        ...
        'RT': r.rt || '',
        'RW': r.rw || '',
        'Kelurahan/Desa': r.kelurahan || 'Gintungreja',
      ...
      const fileName = `Data_Penduduk_Desa_Gintungreja_${new Date().toISOString().split('T')[0]}.xlsx`;
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      pdf.setFontSize(12);
      pdf.text('KECAMATAN GANDRUNGMANGU - DESA GINTUNGREJA', 148, 20, { align: 'center' });
      pdf.setFontSize(10);
      pdf.setFont('helvetica', 'normal');
      pdf.text('LAPORAN DATA KEPENDUDUKAN DESA GINTUNGREJA', 148, 26, { align: 'center' });
      ...
      const fileName = `Data_Penduduk_Desa_Gintungreja_${new Date().toISOString().split('T')[0]}.pdf`;
      pdf.save(fileName);
              ...
              <div key={resident.id} className="p-3.5 space-y-2.5 bg-white">
                ...
                <div className="text-[10px] text-slate-600 space-y-1">
                  <p className="leading-tight">
                    ...
                    {`${resident.address || ''}, RT ${resident.rt || '-'} RW ${resident.rw || '-'}, ${resident.kelurahan || 'Gintungreja'}`}
                  </p>
