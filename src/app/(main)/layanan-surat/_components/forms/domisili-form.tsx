...
export function DomisiliForm({ isAdmin = false }: { isAdmin?: boolean }) {
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      religion: '',
      originAddress: '',
      domicileAddress: 'Desa Gintungreja, Kec. Gandrungmangu, Kab. Cilacap',
    },
  });
        ...
        <FormSection title="Data Alamat" icon={<User className="h-5 w-5 text-primary" />}>
          ...
          <FormField control={form.control} name="domicileAddress" render={({ field }: { field: any }) => (
            <FormItem className="md:col-span-2">
              ...
              <FormControl><Textarea placeholder="Alamat tempat tinggal saat ini di Desa Gintungreja" {...field} disabled={isSubmitting} className="uppercase rounded-2xl" /></FormControl>
