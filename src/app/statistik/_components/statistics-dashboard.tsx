...
export function StatisticsDashboard() {
  ...
  const handleDownloadExcel = () => {
    ...
    XLSX.writeFile(wb, `Statistik_Gintungreja_${filterDusun}.xlsx`);
  ...
  const handleDownloadPDF = () => {
    const doc = new jsPDF();
    doc.setFontSize(18);
    doc.text('Grafik & Statistik Kependudukan Desa Gintungreja', 14, 22);
    doc.setFontSize(11);
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-6 p-10 bg-slate-900 rounded-[3rem] text-white">
        <div className="space-y-1 text-center md:text-left">
          ...
          <p className="text-sm font-medium text-slate-400 italic leading-relaxed">
            Sumber Data: Sistem Informasi Desa (SID) Gintungreja Digital. <br className="hidden sm:block" />
