...
export default function PublicBeritaListPage() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-8 space-y-10">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          {isLoading ? (
            ...
            <Link key={item.id} href={`/BeritaDesa/detail?id=${item.id}`} className="group">
        ...
        <div className="grid lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-8">
            {isLoading ? (
              ...
              <Link href={`/BeritaDesa/detail?id=${headline.id}`} className="group relative block rounded-[2.5rem] overflow-hidden shadow-2xl h-[500px] border-4 border-white">
                <img
            ...
            <Card className="rounded-[2rem] border-none bg-primary text-white overflow-hidden shadow-xl">
              <CardContent className="p-8 space-y-6">
                <div className="flex items-center justify-between border-b border-white/10 pb-4">
                  <h3 className="text-lg font-semibold uppercase font-display italic">Info <span className="text-emerald-200">Penting</span></h3>
                  <TrendingUp className="h-5 w-5 text-white" />
                </div>
                <div className="space-y-4">
                  {trending.slice(2, 5).map((item, i) => (
                    <Link key={item.id} href={`/BeritaDesa/detail?id=${item.id}`} className="flex gap-4 group">
                      <span className="text-2xl font-black text-white/40 italic group-hover:text-white">0{i + 1}</span>
                      <div className="space-y-1">
                        <h4 className="text-[11px] font-bold uppercase leading-tight line-clamp-2 font-sans group-hover:text-emerald-200">
                          {item.title}
                        </h4>
                        <p className="text-[9px] font-bold text-white/60 uppercase font-sans">{item.date}</p>
                      </div>
                    </Link>
                  ))}
                </div>
                ...
                return (
                  <div key={item.id} className="group bg-white p-6 rounded-[2.5rem] border border-slate-200/80 hover:shadow-xl transition-all duration-300 space-y-4 font-sans">
                    ...
                    <div className="space-y-3 pt-2">
                      ...
                      <Link href={`/BeritaDesa/detail?id=${item.id}`}>
            ...
            <div className="lg:col-span-4">
              <Card className="rounded-[2rem] border-none overflow-hidden bg-accent text-white shadow-xl p-8 space-y-6 text-center sticky top-28">
                ...
                <div className="space-y-2">
                  <h3 className="text-xl font-semibold font-display uppercase leading-tight">Ikuti Akun Resmi <span className="italic">Gintungreja</span></h3>
                  <p className="text-xs text-white/70 font-sans">Update pembangunan desa real-time.</p>
                </div>
                <Button className="bg-secondary text-white font-black uppercase tracking-widest rounded-xl hover:bg-emerald-600 w-full h-12">
                  Hubungi Admin
                </Button>
              </Card>
            </div>
      ...
      {/* FOOTER */}
      <footer className="bg-[#081325] text-slate-400 py-16 mt-auto border-t border-slate-800/80">
        <div className="container mx-auto px-4 text-center">
          <Logo />
          <p className="mt-8 text-[10px] font-bold uppercase tracking-widest text-slate-500">
            © 2026 Pemerintah Desa Gintungreja • Website Resmi Pemerintahan Desa
          </p>
        </div>
      </footer>
    </div>
