...
export async function generateVillageNewsDraft(
  ...
  const cleanTitle = input.title || 'Kegiatan Desa Gintungreja';
  const cleanSubtitle = input.subtitle || 'Pembangunan dan Pelayanan Masyarakat';
  const cleanDate = input.date || 'Hari ini';

  const fallbackDraft = `GINTUNGREJA – Pemerintah Desa Gintungreja, Kecamatan Gandrungmangu, Kabupaten Cilacap terus menggiatkan berbagai agenda pembangunan dan pelayanan prima untuk masyarakat. Pada ${cleanDate}, diselenggarakan kegiatan "${cleanTitle}" yang berfokus pada ${cleanSubtitle}.
...
Pemerintah Desa Gintungreja menegaskan komitmennya untuk terus mendukung inisiatif positif yang berdampak langsung bagi peningkatan kesejahteraan warga di 4 Dusun (Dusun Gintungreja, Dusun Sidakaya, Dusun Kedungwringin, dan Dusun Cihaur).

Melalui sinergi yang solid antara pemerintah desa dan seluruh lapisan masyarakat, program ini diharapkan dapat berjalan secara berkesinambungan demi mewujudkan Desa Gintungreja yang maju, mandiri, dan sejahtera.`;
