# Panduan Integrasi Google Spreadsheet PBB-P2 Desa Gintungreja

Dokumen ini berisi kode **Google Apps Script** dan langkah-langkah untuk menghubungkan data PBB-P2 dari sheet **`BNBA PBB 26`** ke website resmi Desa Gintungreja.
...
```javascript
/**
 * API Pencarian PBB-P2 Desa Gintungreja
 * Sheet: "BNBA PBB 26"
...
function doGet(e) {
  try {
    ...
    if (result) {
      ...
      return output.setContent(JSON.stringify({
        status: 'not_found',
        message: 'Nomor Objek Pajak (NOP) ' + nopQuery + ' tidak ditemukan dalam data PBB Desa Gintungreja.'
      }));
    }
...
2. Jika memilih **Penerapan baru (New deployment)**:
   ...
   - **Deskripsi:** `API PBB Gintungreja v2`
...
1. Buka website Desa Gintungreja di browser Anda: **`/admin/settings`**.
