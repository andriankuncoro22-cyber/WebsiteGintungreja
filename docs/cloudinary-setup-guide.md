...
4. Isi form:
   - **Name**: `desa_Gintungreja` (HARUS sama dengan yang di code)
   - **Unsigned**: Pilih **ON** (sangat penting untuk client-side)
   - **Resource type**: Images
   - **Folder**: `desa-gintungreja/footer-logos` (optional, untuk organisir)
5. Klik **Save**
...
```typescript
const response = await fetch('https://api.cloudinary.com/v1_1/desa-gintungreja/image/upload', {
```

Ganti `desa-gintungreja` dengan Cloud Name Anda.
...
- Pastikan nama preset tepat: `desa_Gintungreja` (case sensitive)
